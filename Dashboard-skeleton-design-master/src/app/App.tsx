...
export default function App() {
  ...
  const finalizarPesagem = async (p, e) => {
    ...
    const { error } = await supabase.from('fat_pesagens').update({ peso_saida: pesoSaida, peso_liquido: pesoLiquido, sacas: qtdSacas, valor_unitario: valUnit, valor_total: valTotal, valor_troco: troco, forma_pagamento: e.target.pag.value, status_pagamento: 'FECHADO' }).eq('id', p.id);
    if (!error) {
      load();
      const doc = new jsPDF();
  ...
  const dataForCharts = useMemo(() => { let base = filt; const now = new Date().toISOString().split('T')[0]; const month = new Date().toISOString().slice(0, 7); const year = new Date().getFullYear().toString(); if (activeKpi === "DIÁRIA") base = filt.filter(p => p.data === now); else if (activeKpi === "MENSAL") base = filt.filter(p => p.data?.startsWith(month)); else if (activeKpi === "ANUAL") base = filt.filter(p => p.data?.startsWith(year)); return base; }, [filt, activeKpi]);
  ...
  if (!session) return <div className="flex h-screen items-center justify-center bg-[#0B0F15]"><div className="w-80 p-6 bg-[#161B23] rounded border border-[#ffffff07]"><Auth supabaseClient={supabase} appearance={{ theme: ThemeSupa }} providers={[]} /></div></div>;
  ...
  if (error) return <div className="flex flex-col h-screen items-center justify-center bg-[#0B0F15] text-red-500"><AlertCircle size={40}/><p>{error}</p><button onClick={load} className="mt-4 underline">Tentar novamente</button></div>;
  ...
  return (
    <div className="flex h-screen bg-[#0B0F15] text-white overflow-hidden">
      <aside className="w-48 border-r border-[#ffffff07] p-4 flex flex-col gap-2 shrink-0">
        ...
        <button onClick={() => supabase.auth.signOut()} className="text-[10px] text-red-500 mt-auto text-left">SAIR DA SESSÃO</button>
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-2 gap-4 h-[220px]">
                    <div className="bg-[#161B23] p-2 rounded border border-[#ffffff07] overflow-hidden">
                        <p className="text-[10px] mb-1">PAGAMENTOS ({activeKpi})</p>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={[{name: 'PIX', value: dataForCharts.filter(p=>p.forma_pagamento==='PIX').reduce((a,b)=>a+(Number(b.valor_total)||0),0)}, {name: 'DINHEIRO', value: dataForCharts.filter(p=>p.forma_pagamento==='DINHEIRO').reduce((a,b)=>a+(Number(b.valor_total)||0),0)}]} innerRadius={35} outerRadius={50} dataKey="value" label={(props) => `${props.name} ${(props.percent * 100).toFixed(0)}%`}>
                    ...
                    <div className="bg-[#161B23] p-2 rounded border border-[#ffffff07] overflow-hidden">
                        <p className="text-[10px] mb-1">PRODUTOS ({activeKpi})</p>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={Object.entries(dataForCharts.reduce((acc, p) => { acc[p.produto] = (acc[p.produto] || 0) + (Number(p.valor_total) || 0); return acc; }, {})).map(([name, value]) => ({ name, value }))} innerRadius={35} outerRadius={50} dataKey="value" label={(props) => `${props.name} ${(props.percent * 100).toFixed(0)}%`}>
                                    {COLORS.map((c, i) => <Cell key={i} fill={c} />)}
                                </Pie>
                                <Tooltip formatter={(v) => `R$ ${Number(v).toLocaleString('pt-BR', {minimumFractionDigits: 2})}`} />
                                <Legend />
