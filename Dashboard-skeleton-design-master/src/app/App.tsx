...
export default function App() {
  ...
  const finalizarPesagem = async (p, e) => {
    ...
    if (!error) {
      ...
      doc.text(`Peso Liq: ${pesoLiquido.toFixed(2)}kg | Total: R$ ${valTotal.toFixed(2)}`, 10, 30);
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-5 gap-2">
              {[ {l: "DIÁRIA", v: `R$ ${dia.toFixed(0)}`}, {l: "PESO TOTAL", v: `${pesoTotal.toFixed(0)}kg`}, {l: "MENSAL", v: `R$ ${mens.toFixed(0)}`}, {l: "ANUAL", v: `R$ ${anu.toFixed(0)}`}, {l: "PESAGENS", v: filt.length} ].map((k, i) => (
                <div key={i} className="bg-[#161B23] p-3 rounded border border-[#ffffff07]">
                  <p className="text-[8px] text-gray-400 uppercase text-gray-400">{k.l}</p>
            ...
            <div className="grid grid-cols-2 gap-4 h-[200px]">
              <div className="bg-[#161B23] p-2 rounded border border-[#ffffff07] flex flex-col">
                ...
                <ResponsiveContainer><PieChart><Pie data={pPag} innerRadius={35} outerRadius={50} dataKey="value">{pPag.map((entry, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer>
              </div>
              <div className="bg-[#161B23] p-2 rounded border border-[#ffffff07] flex flex-col">
                <p className="text-[10px] mb-1">RECEITA POR PRODUTO</p>
                <ResponsiveContainer><PieChart><Pie data={pProd} innerRadius={35} outerRadius={50} dataKey="value">{pProd.map((entry, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer>
              </div>
            </div>
        ...
        {aba === "saida" && (
          <div className="grid gap-4">
            {pesagens.filter(p => p.status_pagamento === 'ABERTO').map(p => (
              <form key={p.id} onSubmit={(e) => finalizarPesagem(p, e)} className="bg-[#161B23] p-4 rounded flex gap-2 items-center border border-[#ffffff07]">
                ...
                <button className="bg-green-600 p-1 px-4 rounded text-xs font-bold">FINALIZAR</button>
              </form>
            ))}
          </div>
        )}
