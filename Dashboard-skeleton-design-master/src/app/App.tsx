...
import { Scale, Loader2, AlertCircle, LogOut } from "lucide-react";
...
export default function App() {
  ...
  async function load() {
    setLoading(true);
    const { data: pD } = await supabase.from('fat_pesagens').select('*');
    const { data: cD } = await supabase.from('controle_caixa').select('saldo_atual').eq('id', 1).single();
    setPesagens(pD || []);
  ...
  // ... (Cole aqui todas as suas funções: registrarEntrada, finalizarPesagem, useMemos, cálculos, etc) ...
  ...
  return (
    <div className="flex h-screen bg-[#0B0F15] text-white overflow-hidden">
      <aside className="w-48 border-r border-[#ffffff07] p-4 flex flex-col gap-2 shrink-0">
        ...
        {/* ... (mantenha o restante dos seus filtros laterais aqui) ... */}
      </aside>
      
      <main className="flex-1 p-6 overflow-y-auto">
        {/* ... (mantenha todo o seu código original de exibição da dashboard, entrada e saída) ... */}
      </main>
    </div>
  );
}
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session) {
        ...
        console.log("Nenhuma sessão ativa. Tentando carregar dados de qualquer forma...");
        load(); 
        setLoading(false);
      }
    });
  ...
  const getNextComprovante = async () => {
    const { data } = await supabase
      .from('fat_pesagens')
      .select('comprovante')
      .order('comprovante', { ascending: false })
      .limit(1);
  ...
  const finalizarPesagem = async (p, e) => {
    ...
    if (!error) {
      ...
      const info = [
        ...
        `Placa: ${p.placa}`, 
        `Peso Entrada: ${p.peso_entrada}kg`, 
        `Peso Saida: ${pesoSaida}kg`, 
        `Peso Liquido: ${pesoLiquido.toFixed(2)}kg`, 
        `Qtd Sacas: ${qtdSacas.toFixed(2)}`, 
        `Valor p/ Saca: R$ ${valUnit.toFixed(2)}`, 
        `Valor Total: R$ ${valTotal.toFixed(2)}`, 
        `Pagamento: ${e.target.pag.value}`
      ];
        ...
        {aba === "dashboard" && (
            <div className="flex flex-col gap-6">
                <div className="grid grid-cols-6 gap-2">
                  {[ 
                    {l: "DIÁRIA", v: `R$ ${dia.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`}, 
                    {l: "PESO TOTAL", v: `${pesoTotal.toLocaleString('pt-BR')}kg`}, 
                    {l: "MENSAL", v: `R$ ${mens.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`}, 
                    {l: "ANUAL", v: `R$ ${anu.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`}, 
                    {l: "TROCO PAGO", v: `R$ ${totalTroco.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`}, 
                    {l: "TOTAL", v: filt.length} 
                  ].map((k, i) => (
