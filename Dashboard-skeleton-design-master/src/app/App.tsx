...
export default function App() {
  ...
  const finalizarPesagem = async (p, e) => {
    ...
    const { error } = await supabase.from('fat_pesagens').update({ peso_saida: pesoSaida, peso_liquido: pesoLiquido, sacas: qtdSacas, valor_unitario: valUnit, valor_total: valTotal, forma_pagamento: e.target.pag.value, status_pagamento: 'FECHADO' }).eq('id', p.id);
    if (!error) {
      load();
      const doc = new jsPDF();
        ...
        {aba === "saida" && (
          <div className="flex flex-col gap-4">
            ...
            {pesagens.filter(p => p.status_pagamento === 'ABERTO').map(p => (
              <form key={p.id} onSubmit={async (e) => { e.preventDefault(); const valorTotal = ( (Number(e.target.peso_saida.value) - p.peso_entrada) / 60 ) * Number(e.target.valor_saca.value); const recebido = Number(e.target.recebido.value) || 0; if (e.target.pag.value === "DINHEIRO" && recebido >= valorTotal) { await updateSaldoCaixa(saldoCaixa - (recebido - valorTotal)); } finalizarPesagem(p, e); }} className="bg-[#161B23] p-4 rounded flex flex-col gap-2 border border-[#ffffff07]">
