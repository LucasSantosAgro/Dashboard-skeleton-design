...
export default function App() {
  ...
  if (!session) return (
    <div className="flex h-screen items-center justify-center bg-[#0B0F15] text-white">
      <form onSubmit={async (e) => { e.preventDefault(); await supabase.auth.signInWithPassword({ email, password }); }} className="bg-[#161B23] p-8 rounded border border-[#ffffff07] flex flex-col gap-4 w-80">
        <h2 className="font-bold text-center">Login Operador</h2>
        <input type="email" placeholder="E-mail" className="bg-[#1A2030] p-2 rounded" onChange={e => setEmail(e.target.value)} required />
        <input type="password" placeholder="Senha" className="bg-[#1A2030] p-2 rounded" onChange={e => setPassword(e.target.value)} required />
    ...
    <div className="flex h-screen bg-[#0B0F15] text-white overflow-hidden">
      ...
      <main className="flex-1 p-4 flex flex-col gap-4 overflow-hidden">
        {aba === "dashboard" ? (
          ...
          <form onSubmit={async (e) => { e.preventDefault(); await supabase.from('fat_pesagens').insert([{ placa: e.target.placa.value, produto: e.target.prod.value, peso_entrada: e.target.peso.value, status_pagamento: 'aberto' }]); alert("Entrada registrada!"); }} className="bg-[#161B23] p-6 rounded flex flex-col gap-4">
          ...
          <div className="grid gap-2">
            {pesagens.filter(p => p.status_pagamento === 'aberto').map(p => (
              <div key={p.id} className="bg-[#161B23] p-4 rounded border border-[#ffffff07] flex gap-2 items-center">
                ...
                <select id={`p-${p.id}`} className="bg-[#1A2030] p-1 rounded"><option>PIX</option><option>DINHEIRO</option></select>
                <button onClick={async () => {
                  ...
                  await supabase.from('fat_pesagens').update({ peso_saida: s, peso_liquido: liq, valor_total: tot, forma_pagamento: document.getElementById(`p-${p.id}`).value, status_pagamento: 'finalizado' }).eq('id', p.id);
                  const doc = new jsPDF(); doc.text(`Recibo: ${p.id}`, 10, 10); doc.text(`Liquido: ${liq}kg`, 10, 20); doc.text(`Total: R$ ${tot.toFixed(2)}`, 10, 30); doc.save(`recibo_${p.id}.pdf`);
