      ...
      <main className="flex-1 p-4 flex flex-col gap-4 overflow-hidden">
        ...
        <div className="bg-[#161B23] rounded border border-[#ffffff07] p-3 flex-1 overflow-y-auto">
          <table className="w-full text-left text-[10px]">
            ...
            <tbody>
              {filt.slice(-10).map((p, i) => (
                <tr key={i} className="border-b border-[#ffffff05]">
                  <td className="p-2">{p.data}</td><td className="p-2">{p.comprovante}</td><td className="p-2">{p.produto}</td>
                  <td className="p-2">{p.peso_liquido}kg</td><td className="p-2 text-green-400 font-bold">R$ {Number(p.valor_total).toFixed(0)}</td>
